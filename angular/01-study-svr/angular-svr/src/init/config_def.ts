...
// 参考:https://github.com/redis/node-redis/blob/master/docs/client-configuration.md
export interface ILRSessionRedisStoreOptions {
    /** 连接模式，默认为:redis */
    mode?: 'redis' | 'rediss';
    /** 连接的端口号，默认为:6379 */
    port?: number;
    /** 连接服务器的地址，默认为: localhost */
    host?: string;
    /** 连接的数据库，redis一般有16个数据库，分别是0,1,...15, 默认为: 0 */
    database?: number;
    /** 连接用的用户名，默认为undefined 只有当redis设置了用户名的时候，才需要设置这个 */
    username?: string;
    /** 连接用的密码，默认为undefined, 只有当redis设置了需要密码的时候，才需要设置这个 */
    password?: string;
    /** redis连接名称，在redis服务器可以显示的连接名称，默认为undefined */
    name?: string;
    /** session存在redis超时的时间，单位秒，默认30分钟 */
    ttl?: number;
    /** session存redis的key前缀 */
    prefix?: string;
}
...
export interface ILRConfigSession {
     ...
     */
    redisOptions?: ILRSessionRedisStoreOptions;
