///MD5生成算法
/**
 * 这个是MD5生成算法，有网上流传的C语言版本，去除宏定义后而成。
 *
 * zdhsoft 2011 版权所有
 *
 * @file xmd5.h
...
namespace zdh
{
    namespace detail
    {
        ...
        public:
            ...
            //取生成的MD5字符串
            XChar * GetMD5String(XChar paramMD5[33])
            {
                XByte btMD5[16];
                GetMD5Binary(btMD5);
                XChar * p = paramMD5;
                ...
                {
                    XByte btTmp = btMD5[i];
                    *p++=GetChar((XByte)((btTmp>>4)&0xf));
                    *p++=GetChar((XByte)(btTmp&0xf));               
                }
                *p = 0;
                return paramMD5;
            }
        private:
            ...
            XChar GetChar(XByte paramValue)
            {
                if(paramValue<10) 
                {
        ...
        template<class T>
        XChar * CreateMD5String(const T * paramData, XInt paramLength, XChar paramMD5[33])
        {
            ZDH_ASSERT(paramData != NULL);
            ZDH_ASSERT(paramLength > 0);
            XMD5Impl<> md5;
    ...
    inline XChar * CreateMD5(const XChar * paramString, XChar paramMD5[33])
    {
        ...
        return detail::CreateMD5String(paramString, string_utils::Length(paramString), paramMD5);
    }
    /**
     * 生成MD5串
     * 
    ...
    {
        ZDH_ASSERT(paramString != NULL);
        ZDH_ASSERT(string_utils::Length(paramString) == paramLength);
