#ifndef _X_DATETIME_H_
#define _X_DATETIME_H_
...
namespace zdh
{
    ...
    private:
        ...
        void ResetErrorMsg()
        {
            ...
            case E_DATETIME_INVALIDATION_YEAR:
                strMsg << "Invalidation Year:" << m_InvalidValue;
                break;
            case E_DATETIME_INVALIDATION_MONTH:
                strMsg << "Invalidation Month:" << m_InvalidValue;
                break;
            case E_DATETIME_INVALIDATION_DAY:
                strMsg << "Invalidation Day:" << m_InvalidValue;
                break;
            case E_DATETIME_INVALIDATION_HOUR:
                strMsg << "Invalidation Hour:" << m_InvalidValue;
                break;
            case E_DATETIME_INVALIDATION_MINUTE:
                strMsg << "Invalidation Minute:" << m_InvalidValue;
                break;
            case E_DATETIME_INVALIDATION_SECOND:
                strMsg << "Invalidation Second:" << m_InvalidValue;
                break;
            case E_DATETIME_INVALIDATION_MILLIS:
                strMsg << "Invalidation Millis:" << m_InvalidValue;
                break;
            case E_DATETIME_INVALIDATION_DATETIME:
                strMsg << "Invalidation DateTime";
                break;
            case E_DATETIME_INVALIDATION_DAYS:
                strMsg << "Invalidation Days:" << m_InvalidValue;
                break;
            case E_DATETIME_OUT:
                strMsg << "Out of DateTime";
                break;
            case E_DATETIME_DAYS_OUT:
                strMsg << "Out of Days" << m_InvalidValue;
                break;
            case E_DATETIME_TIMES_OUT:
                strMsg << "Out of Times" << m_InvalidValue;
                break;
            default:
                strMsg << "Unknow DateTime Error:" << m_InvalidValue ;
                break;
            }
    ...
    namespace detail
    {
        ...
        inline XLong GetNowDateTime()
        {
                ...
                XLong lngNow = tv.tv_sec;
                lngNow *= 1000;
                lngNow += tv.tv_usec / 1000;
                lngNow += (DAYS_1970_1_1 * MILLIS_PRE_DAY64 - - (timezone * MILLIS_PRE_SECOND));
                return lngNow;
            #endif
        }
        ...
        */
        inline void GetSystemDateTime(SSystemDateTime & paramST)
        {
            ...
            struct timeval tv;
            gettimeofday(&tv, NULL);
            paramST.Milliseconds = tv.tv_usec / 1000;
            XInt tmp = tv.tv_sec;
            paramST.Second = tmp % 60;
            tmp /= 60;
            paramST.Minute = tmp % 60;
            tmp /= 60;
            paramST.Hour = tmp % 24;
            tmp /= 24;
            tmp += DAYS_1970_1_1;
            ...
            paramST.Year = iYear;
            paramST.Month = iMonth;
            paramST.Day = iDay;
            paramST.WeekDay = tmp % 7;
            paramST.Zone = timezone / 60;
        #endif
        }
        ...
        */
        ...
        #elif defined(OS_LINUX)
            struct timeval tv;
            tv.tv_sec = (CalcDays(paramST.Year,paramST.Month,paramST.Day) - DAYS_1970_1_1) * 86400 + paramST.Hour * 3600 + paramST.Minute * 60 + paramST.Second;
            tv.tv_usec = paramST.Milliseconds * 1000;
    ...
    */
    ...
    inline void DateTimeMillisToDateTime(XLong paramMillisCount, XInt &paramYear, XInt & paramMonth, XInt & paramDay,
        ...
        ZDH_THROW(EDateTimeError)
    {
        ZDH_ASSERT(!(paramMillisCount < MIN_MILLIS || paramMillisCount >= MAX_MILLIS ));
...
#if defined(OS_WINDOWS)
        static const XWideChar * WeekString_2[5][7] =
        {
            {L"Sunday", L"Monday", L"Tuesday", L"Wednesday", L"Thursday", L"Friday", L"Saturday"},
            {L"星期日", L"星期一", L"星期二", L"星期三", L"星期四", L"星期五", L"星期六"},
            {L"Sun.", L"Mon.", L"Tues.", L"Wed.", L"Thurs.", L"Fri.", L"Sat."},
            {L"0", L"1", L"2", L"3", L"4", L"5", L"6"},
            {L"日", L"一", L"二", L"三", L"四", L"五", L"六"}
        };
#endif
#if defined(OS_WINDOWS)
        if( sizeof(StringChar) == 1 )
        {
            paramString =  (const StringChar *)WeekString_1[aMode][m_WeekDay];
        }
        else
        {
            paramString =  (const StringChar *)WeekString_2[aMode][m_WeekDay];
        }
#else
        paramString =  (const StringChar *)WeekString_1[aMode][m_WeekDay];
#endif
